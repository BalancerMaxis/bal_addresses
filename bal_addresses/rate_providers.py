...
class RateProviders:
    ...
    chain_ids_by_name = AddrBook.chain_ids_by_name
    source_data = requests.get(f"{GITHUB_CODEREVIEW_RAW}/rate-providers/registry.json").json()

    def __init__(self, chain):
        self.chain = chain
        if chain not in self.chain_ids_by_name:
            ...
            self.rate_providers_by_token = Munch(self.translate_source_data(self.info_by_rate_provider))
    ...
    def get_review_for_safe_rate_provder(self, token_address) -> Optional[str]:
        ...
        """
        token_info = self.rate_providers_by_token.get(to_checksum_address(token_address))
    ...
    def translate_source_data(self, chain_data):
        ...
        for provider, infodict in chain_data.items():
            ...
            this.review_link = infodict.get("review").replace("./", f"{GITHUB_CODEREVIEW_NICE}/rate-providers/")
